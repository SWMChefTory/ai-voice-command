...
class VoiceCommandClient(ABC):
    @abstractmethod
    async def send_result(self, stt_provider: STTProvider, result: Intent, user_id: UUID, session_id: str):
        pass
    ...
    async def send_result(self, stt_provider: STTProvider, result: Intent, user_id: UUID, session_id: str):
        ...
        try:
            async with httpx.AsyncClient() as client:
                await client.post(
                url,
                json=VoiceCommandRequest.from_intent(result, user_id, stt_provider, session_id).model_dump()
            )
