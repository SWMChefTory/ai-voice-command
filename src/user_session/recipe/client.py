...
from .models import RecipeCaption, RecipeStep
from .schema import RecipeCaptionsResponse, RecipeStepsResponse
...
class RecipeCheftoryClient(RecipeClient):
    ...
    async def get_recipe_caption(self, recipe_id: UUID) -> List[RecipeCaption]:
        async with httpx.AsyncClient() as client:
            response = await client.get(f"{self.api_base}/papi/v1/recipes/{recipe_id}/caption")
            response.raise_for_status()
            recipe_caption_response = RecipeCaptionsResponse.model_validate(response.json())
            return [RecipeCaption.from_response(segment) for segment in recipe_caption_response.captions]
